...
titleSize = 4
fsize = 10
source("~/Office Codes//RCodesStreetSmart//EMGAnom//multiplot.R")
plot <- c()

emgx <-  syntheticemg(n.length.out=4096,
                       on.sd=2, on.duration.mean=1000, on.duration.sd=1,
                       off.sd=0.05, off.duration.mean=350, off.duration.sd=1,
                       on.mode.pos=0.75,shape.factor=0.5, samplingrate=1000,
                       units="mV", data.name="Synthetic EMG")

normalData <- data.frame(data = emgx$values)
normalData$x <- seq(1: nrow(normalData)) 
labelData <- rep(c(rep("Silent Phase",350),rep("Activity Phase",1000)),ceiling(4096/1350))[1:4096]
normalData$label <- labelData
g <- ggplot(normalData,aes(x = x,y=data))
plot[[1]]<-g+geom_line() + 
  scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4")) + 
  xlab("Time (sec)") + ylab("EMG (mv)") + 
  labs(title="(A)")  +theme_bw(base_size = fsize)
whiteNoise <- rnorm(4096,mean = 0,sd =0.5)
whiteNoiseEMG <- emgx$values + whiteNoise
wnData <- data.frame(data = whiteNoiseEMG)
wnData$x <- seq(1: nrow(wnData)) 
g <- ggplot(wnData,aes(x = x,y=data))
plot[[2]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4"))+
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(B)") + 
  theme(plot.title = element_text(size = rel(titleSize)))+theme_bw(base_size = fsize)

correlatedNoise <- diffinv(rnorm(4095,mean = 0,sd = 0.03))
corNoiseEmg <- emgx$values + correlatedNoise
cnData <- data.frame(data = corNoiseEmg)
cnData$x <- seq(1: nrow(cnData)) 
g <- ggplot(cnData,aes(x = x,y=data))
plot[[3]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4")) + 
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(C)")+
  theme(plot.title = element_text(size = rel(titleSize))) +theme_bw(base_size = fsize)

# t<- seq(from = 1, to = 1.8,length.out = 2000)

datECG <- read.table("~/Personal/Data Sets/MHEALTHDATASET/mHealth_subject1.log", sep="\t")
datECGnoise <- datECG$V4[1:4096]
rampAnom <- emgx$values + 0.75*datECGnoise

# rampStart <- floor(runif(1,1,2000))
# rampAnom[rampStart:(rampStart+1999)] <- emgx$values[rampStart:(rampStart+1999)] + t
rampData <- data.frame(data = rampAnom)
rampData$x <- seq(1: nrow(rampData)) 
#rampData$label <- c(rep("Normal",(rampStart-1)),rep("Ramp Anomaly",2000),rep("Normal",(4096-rampStart-1999)))
g <- ggplot(rampData,aes(x = x,y=data))
plot[[4]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4")) +
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(D)")+ 
  theme(plot.title = element_text(size = rel(titleSize))) +theme_bw(base_size = fsize)

tp <- seq(from = 0,to = 4,length.out = 4096)


# t<- seq(from = 1, to = 1,length.out = 2000)
# stepStart <- floor(runif(1,1,2000))
powerLineNoise <- 0.5*sin(60*2*pi*tp) + 0.5*sin(360*2*pi*tp)
ampl <- 0.3
stepAnom <- emgx$values +  ampl*powerLineNoise
# stepAnom[stepStart:(stepStart+1999)] <- emgx$values[stepStart:(stepStart+1999)] + t
stepData <- data.frame(data =stepAnom)
stepData$x <- seq(1: nrow(stepData)) 
g <- ggplot(stepData,aes(x = x,y=data))
plot[[5]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4"))  +
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(E)")+ 
  theme(plot.title = element_text(size = rel(titleSize)))+theme_bw(base_size = fsize)

emgx <-  syntheticemg(n.length.out=4096,
                       on.sd=2, on.duration.mean=1000 + runif(1,-500,500), on.duration.sd=1,
                       off.sd=0.05, off.duration.mean=350, off.duration.sd=1,
                       on.mode.pos=0.75,shape.factor=0.5, samplingrate=1000,
                       units="mV", data.name="Synthetic EMG")
activityAnom <- emgx$values
acData <- data.frame(data =activityAnom)
acData$x <- seq(1: nrow(acData)) 
g <- ggplot(acData,aes(x = x,y=data))
plot[[6]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4"))  +
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(F)") + 
  theme(plot.title = element_text(size = rel(titleSize)))+theme_bw(base_size = fsize)


emgx <-  syntheticemg(n.length.out=4096,
                       on.sd=2, on.duration.mean=1000, on.duration.sd=1,
                       off.sd=0.05, off.duration.mean=350+runif(1,-200,200), off.duration.sd=1,
                       on.mode.pos=0.75,shape.factor=0.5, samplingrate=1000,
                       units="mV", data.name="Synthetic EMG")
silentAnomaly <- emgx$values
siData <- data.frame(data =silentAnomaly)
siData$x <- seq(1: nrow(siData)) 
g <- ggplot(siData,aes(x = x,y=data))
plot[[7]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4"))+
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(G)")+ 
  theme(plot.title = element_text(size = rel(titleSize)))+theme_bw(base_size = fsize)
#ggsave(file="name.eps")
t1<- seq(from = 1, to = 1,length.out = 2000)
t2<- seq(from = 1, to = 1.8,length.out = 2000)
whiteNoise <- rnorm(4096,mean = 0,sd =0.5)
correlatedNoise <- diffinv(rnorm(4095,mean = 0,sd = 0.03))

# stepStart <- floor(runif(1,1,2000))
# rampStart <- floor(runif(1,1,2000))
combineNoise <- emgx$values + whiteNoise + 0.25*powerLineNoise +  0.25*datECGnoise

siData <- data.frame(data =combineNoise)
siData$x <- seq(1: nrow(siData)) 
g <- ggplot(siData,aes(x = x,y=data))
plot[[8]]<-g+geom_line() + scale_x_continuous(breaks=c(1000,2000,3000,4000), labels=c("1", "2", "3","4"))+
  xlab("Time (sec)") + ylab("EMG (mv)") + ggtitle("(H)")+
  theme(plot.title = element_text(size = rel(titleSize))) +theme_bw(base_size = fsize)
